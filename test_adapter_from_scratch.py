...
from settings import MEMORY_FACTOR, LEN_FACTOR, TASK_DICT, MODEL_CONFIG, DATA_ATTRS, SPECIAL_TOKENS, CONFIG_CLASS, CONFIG_NAME, SAVE_NAME, FINAL_SAVE_NAME
...
def test_one_to_one(task_load, task_eval, model, score_dict,test_run):
...
#                             import pdb;pdb.set_trace();
                            all_pasts[layer_id][cnt] = torch.stack([each_key_value[i, :, :len_cqs[i], :] for each_key_value in pasts[layer_id]]).type(torch.float32 if args.fp32 else torch.half)
            cnt += 1
...
def test_one_to_many(task_load):
    ...
    adapter_name = task_load.replace(".", "_")   ##Change

...
def train(task_ids, model,train_type=0):
    ...
    if task_ids[0]>0 and train_type==1:
        ...
        #                 )adapter_config = ConfigUnion(
        adapter_config = ConfigUnion(
                            PrefixTuningConfig(cross_prefix=True, prefix_length=30, bottleneck_size=800,non_linearity='relu',leave_out=[0,2,4,6,8,10]),
                            AdapterConfig(mh_adapter=False, output_adapter=True, reduction_factor=4, non_linearity="relu", init_weights="mam_adapter", leave_out=[0,2,4,6,8,10]),
                        )
